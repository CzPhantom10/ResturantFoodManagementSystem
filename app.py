...
class Chef:
    ...
    def assign_order(self, order):
        ...
        while order.remaining_time > 0:
            time.sleep(1)
            order.remaining_time -= 1
            st.session_state['order_manager'].update_order_status(order)
...
class OrderManager:
    ...
    def update_order_status(self, order):
        ...
        self.orders[order_index] = order  # Update the specific order
...
if st.session_state['order_collected']:
    ...
    while True:
        ...
        for order in orders:
            st.write(f"Order ID: {order.order_id}, Customer: {order.customer_name}, "
                    f"Food: {order.food_item}, Remaining Time: {order.remaining_time}s, Status: {order.status}")
        time.sleep(1) 
    if st.button("Reset"):
        # Clear session state
